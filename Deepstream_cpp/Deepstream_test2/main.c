    ...
    gpointer u_data)
{
    ...
    for (l_frame = batch_meta->frame_meta_list; l_frame != NULL;
        ...
        printf("num person: %i, num vehicle %i", person_count, vehicle_count);
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2)
    {
        g_printerr("Usage %s app h264 stream", argv[0]);
        return -1;
    }

    /* Gstreamer initiliazation*/
    gst_init(&argc, &argv);
    ...
    /* Source element to read from files*/
    source = gst_element_factory_make("filesrc", "file-source");

    /* h264 parser to parse stream*/
    ...
    /* Tracker to track identited object*/
    nvtracker = gst_element_factory_make("nvtracker", "tracker");

    /* Create 3 secondary gpu infer engine*/
    sgie1 = gst_element_factory_make("nvinfer", "sgie1-nvinfer");

    sgie2 = gst_element_factory_make("nvinfer", "sgie2-nvinfer");

    sgie3 = gst_element_factory_make("nvinfer", "sgie3-nvinfer");

    /* Convert NV12 to RGBA*/
    nvvidconv = gst_element_factory_make("nvvideoconvert", "nvvideo-convertor");
