...
def make_nodi(index, layers, detection_param, box_size_param):
    ...
    # print("box_layer.buffer: ", box_layer.buffer)

    # https://forums.developer.nvidia.com/t/urgent-how-to-convert-deepstream-tensor-to-numpy/128601/9
    box_ptr = ctypes.cast(pyds.get_ptr(box_layer.buffer), ctypes.POINTER(ctypes.c_float))
    box_vector = np.ctypeslib.as_array(box_ptr, shape=(8732*4, ))
...
def nvds_infer_parse_custom_tf_ssd(output_layer_info, detection_param, box_size_param,
    ...
    # print(type(box_layer), type(class_layer), type(num_detection_layer), type(score_layer))
