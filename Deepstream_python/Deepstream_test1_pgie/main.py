    ...
    while l_frame is not None:
        ...
        while l_user is not None:
            ...
            for i in range(tensor_meta.num_output_layers):
                ...
                box_ptr = ctypes.cast(pyds.get_ptr(layer_buffer), ctypes.POINTER(ctypes.c_float))
                if layer_name == "conv2d_bbox":
                    box_vector = np.ctypeslib.as_array(box_ptr, shape=(int(1*NUM_CLASSES*INPUT_HEIGHT/4*INPUT_WIDTH/4), ))
                else:
                    box_vector = np.ctypeslib.as_array(box_ptr, shape=(int(1*NUM_CLASSES*4*INPUT_HEIGHT/4*INPUT_WIDTH/4), ))
                print(box_vector.shape, box_vector[:10])
